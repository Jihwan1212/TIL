...
def count_laser_pieces():
    ...
    for tc in range(1, T + 1):
        ...
        while i < len(arrangement):
            if arrangement[i] == '(':
                if i + 1 < len(arrangement) and arrangement[i + 1] == ')':
                    pieces += stack_size  # 레이저가 쇠막대기를 자름
                    i += 1  # 레이저는 두 문자이므로 다음 문자로 이동
                else:
                    stack_size += 1  # 새로운 쇠막대기 시작
            else:
                stack_size -= 1  # 쇠막대기 끝남
                pieces += 1  # 하나의 쇠막대기 조각 추가
            i += 1
