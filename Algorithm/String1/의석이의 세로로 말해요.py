...
def read_words_vertically():
    T = int(input().strip())  # 테스트 케이스 개수 입력

    for tc in range(1, T + 1):
        words = [input() for _ in range(5)]  # 5개의 단어 입력 받기
        max_length = max(len(word) for word in words)  # 가장 긴 단어의 길이 계산

        result = []  # 세로로 읽은 문자를 저장할 리스트

        for i in range(max_length):  # 각 열을 순회
            for word in words:  # 각 단어를 순회
                if i < len(word):  # 단어의 길이를 초과하지 않는 경우
                    result.append(word[i])  # 세로로 읽은 문자 추가

        print(f"#{tc} {''.join(result)}")  # 리스트를 문자열로 변환하여 출력


# 실행
read_words_vertically()
